...
const getEsmModuleMap = memorize(() => {
	try {
		// @ts-expect-error
...
const enableActiveTracking = () => {
	if (supportsEsm) {
		...
		try {
			...
			throw new Error(
				"To enable active tracking for ESM you need to pass `--expose-internals` to node.\n" +
...
class FileSystemInfo {
	...
	constructor(fs, { managedPaths = [], immutablePaths = [], logger } = {}) {
		...
		this._warnedAboutExposeInternals = false;
		this._warnedAboutUndetectedModuleSystem = false;
				...
				switch (type) {
					...
					case RBDT_FILE_DEPENDENCIES: {
						...
						if (supportsEsm) {
										...
										.then(dependencies => {
											if (dependencies && dependencies.length > 0) {
												...
												for (let i = 0; i < dependencies.length; i++) {
													...
													console.log(path + " -> " + childPath + " (ESM)");
													queue.push({
														type: RBDT_FILE,
														path: childPath
													});
							...
							} else if (!this._warnedAboutExposeInternals) {
								this.logger.warn(
									"To track dependencies of EcmaScript Modules in the Node.js dependency graph access to Node.js internals is required.\n" +
										"Run node with `node --expose-internals` to grant access to the ESM dependency graph.\n" +
										"E. g. run `node --expose-internals node_modules/webpack/bin/webpack.js ...`.\n" +
										"Without access to internals, webpack has to guess dependencies and this can lead to incorrect tracking."
								);
								this._warnedAboutExposeInternals = true;
							}
						}

						if (!this._warnedAboutUndetectedModuleSystem) {
							this.logger.debug(
								`Unable to get dependencies for ${path} from module system.\n` +
									"We assume that dependencies stay in the directory and add the whole directory as dependencies.\n" +
									"Note that this is not optimal and might lead to issues."
							);
							this._warnedAboutUndetectedModuleSystem = true;
						} else {
							this.logger.debug(
								`Unable to get dependencies for ${path} from module system.`
							);
...
module.exports.enableActiveTracking = enableActiveTracking;
